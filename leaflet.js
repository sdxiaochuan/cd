var COPYRIGHT = "Leaflet, a JavaScript library for mobile-friendly interactive maps. http://leafletjs.com (c) 2010-2013, Vladimir Agafonkin (c) 2010-2011, CloudMade (c) Changes by Dzwiedz, Advameg, Inc";
...
L.LatLngBounds.prototype.intersects = function(bounds) {
...
L.LatLngBounds.prototype.equals = function(bounds) {
    var sw = this._southWest,
        ne = this._northEast,
        sw2 = bounds.getSouthWest(),
        ne2 = bounds.getNorthEast();
    return sw.equals(sw2) && ne.equals(ne2);
};
L.LatLngBounds.prototype.pad = function(bufferRatio) {
    var sw = this._southWest,
        ne = this._northEast,
        hb = Math.abs(sw.lat - ne.lat) * bufferRatio,
        wb = Math.abs(sw.lng - ne.lng) * bufferRatio;
    return new L.LatLngBounds(new L.LatLng(sw.lat - hb, sw.lng - wb), new L.LatLng(ne.lat + hb, ne.lng + wb));
};
L.LatLngBounds.prototype.shrink = function(sub) {
    var sw = this._southWest,
        ne = this._northEast;
    var subLat = sub / 200.0 * Math.abs(sw.lat - ne.lat);
    var subLng = sub / 200.0 * Math.abs(sw.lng - ne.lng);
    return new L.LatLngBounds(new L.LatLng(sw.lat + subLat, sw.lng + subLng), new L.LatLng(ne.lat - subLat, ne.lng - subLng));
};
L.Map.prototype.openPopupOrig = L.Map.prototype.openPopup;
L.Map.prototype.openPopup = function(popup) {
    if (typeof this.skipPopup != 'undefined' && this.skipPopup) return;
    this.openPopupOrig(popup);
};
L.Map.prototype.closePopupOrig = L.Map.prototype.closePopup;
L.Map.prototype.closePopup = function() {
    if (this._popupSkipClose) {
        this._popupSkipClose = false;
        return this;
    }
    this.closePopupOrig();
}
L.Map.ScrollWheelZoom.prototype._onWheelScrollOrig = L.Map.ScrollWheelZoom.prototype._onWheelScroll;
L.Map.ScrollWheelZoom.prototype._onWheelScroll = function(e) {
    if (typeof this._map.interactionMode != 'undefined' && !this._map.interactionMode) {
        L.DomEvent.preventDefault(e);
        L.DomEvent.stopPropagation(e);
        return;
    }
    this._onWheelScrollOrig(e);
}
L.FeatureGroup.prototype.hideLayerGroup = function() {
    if (typeof DEL_LAYERS != 'undefined' && !DEL_LAYERS) {
        this.setStyle({
            stoke: false,
            fill: false,
            clickable: false,
    ...
    mL_removePolyMouseEvents(this);
    this._map.removeLayer(this);
}
L.FeatureGroup.prototype.showLayerGroup = function() {
    if (typeof DEL_LAYERS != 'undefined' && !DEL_LAYERS) {
        this.setStyle({
            stoke: true,
            fill: true,
    ...
    mL_addPolyMouseEvents(this);
    this._map.addLayer(this);
}
L.TileLayer.prototype.fire = function(t, e) {}
L.Map.prototype._layerAddOrig = L.Map.prototype._layerAdd;
L.Map.prototype._layerAdd = function(t) {
    if (typeof this.optimizePolies == 'undefined') return this._layerAddOrig(t);
    var tt, e;
    if (typeof t._layers != 'undefined' && (t instanceof L.LayerGroup || t instanceof L.FeatureGroup)) {
        t._map = this;
        for (var i in t._layers) {
            tt = t._layers[i];
            if (typeof tt._mRadius != 'undefined') {
                return this._layerAddOrig(t);
            }
            e = L.stamp(tt);
            if (this._layers[e]) continue;
            this._layers[e] = tt;
            if (typeof tt._initElements == 'undefined') {
                this._layerAdd(tt);
                continue;
            }
            this._loaded && (tt._map = this, tt._container || (tt._initElements(), tt._initEvents()), tt.projectLatlngs(), tt._updatePath(), tt._container && tt._map._pathRoot.appendChild(tt._container));
        }
        return;
    }
    t.onAdd(this);
}
L.Map.prototype.removeLayerOrig = L.Map.prototype.removeLayer;
L.Map.prototype.removeLayer = function(t) {
    if (typeof this.optimizePolies == 'undefined') return this.removeLayerOrig(t);
    var tt, e;
    if (typeof t._layers != 'undefined' && (t instanceof L.LayerGroup || t instanceof L.FeatureGroup)) {
        for (var i in t._layers) {
            tt = t._layers[i];
            if (typeof tt._mRadius != 'undefined') {
                return this.removeLayerOrig(t);
            }
            if (typeof tt._container == 'undefined') {
                this.removeLayer(tt);
                continue
            }
            this._pathRoot.removeChild(tt._container), tt._map = null, L.Browser.vml && (tt._container = null, tt._stroke = null, tt._fill = null);
            if (0 && typeof tt._fillColor != 'undefined') {
                tt._fillColor = null;
                tt._parts = null;
                tt._holePoints = null;
                tt._originalPoints = null;
            }
            e = L.stamp(tt);
    ...
    this.removeLayerOrig(t);
}
L.Path.prototype.onAddOrig = L.Path.prototype.onAdd;
L.Path.prototype.onAdd = function(t) {
    if (!t || typeof t.optimizePolies == 'undefined') return this.onAddOrig(t);
    this._map = t, this._container || (this._initElements(), this._initEvents()), this.projectLatlngs(), this._updatePath(), this._container && this._map._pathRoot.appendChild(this._container), this.fire("add");
};
L.Path.prototype.onRemoveOrig = L.Path.prototype.onRemove;
L.Path.prototype.onRemove = function(t) {
    if (!this._map || typeof this._map.optimizePolies == 'undefined') return this.onRemoveOrig(t);
    t._pathRoot.removeChild(this._container), this.fire("remove"), this._map = null, L.Browser.vml && (this._container = null, this._stroke = null, this._fill = null);
};
L.Polyline.prototype._getPathPartStr = function(t) {
    for (var e, i = L.Path.VML, n = 0, s = t.length, a = ""; s > n; n++) {
        e = t[n], i && e._round(), a += (n ? "L" : "M") + e.x + " " + e.y;
        if (Math.abs(e.x) > 10000 || Math.abs(e.y) > 10000) return 'M0 0';
    }
    return a
}
L.Path.prototype._initEventsOrig = L.Path.prototype._initEvents;
L.Path.prototype._initEvents = function() {
    if (!this._map || typeof this._map.optimizePolies == 'undefined') return this._initEventsOrig();
    if (this.options.clickable) {
        ...
        var t = ["mouseover", "mouseout"];
        for (var e = 0; e < t.length; e++) {
            L.DomEvent.on(this._container, t[e], this._fireMouseEvent, this);
        }
    }
}
L.FeatureGroup.prototype.addLayerOrig = L.FeatureGroup.prototype.addLayer;
L.FeatureGroup.prototype.addLayer = function(t) {
    if (typeof this.length == 'undefined') this.length = 0;
    this.length++;
    this.addLayerOrig(t);
}
L.FeatureGroup.prototype.removeLayerOrig = L.FeatureGroup.prototype.removeLayer;
L.FeatureGroup.prototype.removeLayer = function(t) {
    this.length--;
    this.removeLayerOrig(t);
}
L.LayerGroup.prototype.clearLayersOrig = L.LayerGroup.prototype.clearLayers;
L.LayerGroup.prototype.clearLayers = function() {
    this.clearLayersOrig();
    this.length = 0;
}
L.LatLngBounds.prototype.padEx = function(add, adj) {
    var sw = this._southWest,
        ne = this._northEast;
    var diff = (Math.abs(sw.lng - ne.lng) * adj - Math.abs(sw.lng - ne.lng)) / 2;
    var rnd = 100.0;
    var dLat = Math.abs(sw.lat - ne.lat);
    var dLng = Math.abs(sw.lng - ne.lng);
    while (add > dLat / 5 || add > dLng / 5) {
        rnd *= 10;
        add /= 2.0;
    }
    var swLat = Math.floor((sw.lat - add) * rnd) / rnd;
    var swLng = Math.floor((sw.lng - add - diff) * rnd) / rnd;
    var neLat = Math.ceil((ne.lat + add) * rnd) / rnd;
    var neLng = Math.ceil((ne.lng + add + diff) * rnd) / rnd;
    swLat = Math.min(90, Math.max(-90, swLat));
    swLng = Math.min(180, Math.max(-180, swLng));
    neLat = Math.min(90, Math.max(-90, neLat));
    neLng = Math.min(180, Math.max(-180, neLng));
    return new L.LatLngBounds(new L.LatLng(swLat, swLng), new L.LatLng(neLat, neLng));
};
L.mL_Label = L.Path.extend({
    initialize: function(latlng, radius, text, options) {
    ...
    projectLatlngs: function() {
        ...
        if (this._radius > 50) this._radius = 50;
    },
    getPathString: function() {
        var p = this._point,
            r = this._radius
        ...
        if (!L.Path.SVG) {
            var x1 = Math.round(p.x - w / 2),
                x2 = Math.round(p.x + w / 2);
            var y1 = Math.round(p.y - h / 2),
                y2 = Math.round(p.y + h / 2);
            return 'M ' + x1 + ' ' + y1 + ' L ' + x2 + ' ' + y1 + ' L ' + x2 + ' ' + y2 + ' L ' + x1 + ' ' + y2 + ' x';
        } else return '';
    },
    _initPath: function() {
        if (this._text) {
            ...
            if (L.Path.SVG) {
                ...
                this._textLabel.setAttribute("fill", "black");
                this._rect.setAttribute("style", "fill-opacity: 0.8; fill: white; stroke: black;");
                ...
                this._textLabel.setAttribute("style", "font-family: Helvetica; font-size: 10px;");
                this._textLabel.setAttribute("textLength", w - 5);
                this._textLabel.setAttribute("lengthAdjust", "spacing");
                ...
                this._rect.setAttribute("rx", 4);
                this._rect.setAttribute("ry", 4);
            } else if (!L.Path.CANVAS) {
                this._textLabel = this._createElement('textbox');
                this._textContainer = document.createElement("div");
                this._textLabel.appendChild(this._textContainer);
                this._path.appendChild(this._textLabel);
                this._textLabel.fitpath = true;
                this._textLabel.on = true;
                this._textContainer.setAttribute("style", "text-align: center; padding-left: 2px;");
                this._textLabel.setAttribute("v-text-anchor", "middle");
                this._textContainer.innerText = this._text;
                this._textLabel.setAttribute("style", "font-family: Helvetica;");
                this._textContainer.style.textAlign = "center";
                this._textContainer.style.paddingLeft = "2px";
            }
        }
    },
    _updatePath: function() {
        ...
        if (L.Path.SVG) {
            if (this._textLabel) {
                var size = this._radius / 12.0;
                this._container.setAttribute("transform", "translate(" + this._point.x + "," + this._point.y + "),scale(" + size + "," + size + ")");
                this._textLabel.textContent = this._text;
            }
        } else if (!L.Path.CANVAS) {
            var size = this._radius;
            var w = this._width * size,
                h = size;
            var x = this._point.x;
            var y = this._point.y;
            this._textLabel.style.fontSize = size + "px";
            this._textLabel.style.fontFamily = "Helvetica";
            this._textLabel.style.width = w + "px";
            this._textLabel.style.height = h + "px";
            this._textLabel.setAttribute("style", "font-family: Helvetica; width: " + w + "px; height: " + h + "px; font-size: " + size + "px;");
        }
    },
    _initEvents: function() {
        L.Path.prototype._initEvents.call(this);
        if (!L.Path.CANVAS && this.options.clickable) {
            if (!L.Path.SVG) {
                this._textLabel.className = 'leaflet-clickable';
            } else {
                this._textLabel.setAttribute("class", 'leaflet-clickable');
            }
            L.DomEvent.addListener(this._container, 'click', this._onMouseClick, this);
            if (!L.Path.SVG && !L.Path.CANVAS)
                L.DomEvent.addListener(this._textContainer, 'click', this._onMouseClick, this);
            if (!L.Path.CANVAS) {
                var events = ['dblclick', 'mousedown', 'mouseover', 'mouseout', 'mousemove'];
                for (var i = 0; i < events.length; i++) {
                    L.DomEvent.addListener(this._container, events[i], this._fireMouseEvent, this);
                    if (!L.Browser.svg && L.Browser.vml) {
                        L.DomEvent.addListener(this._textContainer, events[i], this._fireMouseEvent, this);
                    }
                }
            }
        }
...
L.mL_CircleMarker = L.Circle.extend({
    options: {
        radius: 10,
        weight: 2,
        txt: 'X',
    ...
    initialize: function(latlng, options) {
        ...
        this._txt = this.options.txt;
        this._txtStyle = this.options.textStyle;
        this._txtClass = this.options.textClass;
    },
    projectLatlngs: function() {
        this._point = this._map.latLngToLayerPoint(this._latlng);
    },
    ...
    _initPath: function() {
        L.Circle.prototype._initPath.call(this);
        if (!L.Path.CANVAS && this._txt) {
            if (L.Browser.svg) {
                this._textLabel = this._createElement('text');
                this._container.appendChild(this._textLabel);
                this._textLabel.setAttribute("fill", "black");
                if (this._txtClass)
                    this._textLabel.setAttribute("class", this._txtClass);
                this._textLabel.setAttribute("text-anchor", "middle");
                this._textLabel.textContent = this._txt;
                this._textLabel.setAttribute("x", 0);
                this._textLabel.setAttribute("y", Math.round(this._radius * 0.35));
                this._textLabel.setAttribute("style", this._txtStyle + " font-size: " + Math.round(this._radius * 1.5) + "px;");
            } else if (L.Browser.vml) {
                this._textLabel = this._createElement('textbox');
                this._textContainer = document.createElement('div');
                this._textLabel.appendChild(this._textContainer);
                this._path.appendChild(this._textLabel);
                this._textContainer.innerText = this._txt;
                this._textLabel.setAttribute("style", this._txtStyle + "  height: " + Math.round(this._radius * 2.5) + "px; width: " + Math.round(this._radius * 2.5) + "px; vertical-align: middle;font-family: Times New Roman; layout-flow: horizontal-ideographic; v-text-anchor:  middle-center; ");
                this._textContainer.setAttribute("style", this._txtStyle + " padding-top:3px; text-align:center;  height: " + Math.round(this._radius * 2.5) + "px; width: " + Math.round(this._radius * 2.5) + "px; vertical-align: middle;font-family: Times New Roman; layout-flow: horizontal-ideographic; v-text-anchor:  middle-center; ");
                this._textContainer.setAttribute("style", this._txtStyle + " padding-top:3px; text-align:center;  height: " + Math.round(this._radius * 2.5) + "px; width: " + Math.round(this._radius * 2.5) + "px; vertical-align: middle;font-family: Times New Roman; layout-flow: horizontal-ideographic; v-text-anchor:  middle-center; ");
                this._textContainer.style.paddingTop = "3px";
                this._textContainer.style.textAlign = "center";
                this._textContainer.style.width = Math.round(this._radius * 2.5) + "px";
                this._textContainer.style.height = Math.round(this._radius * 2.5) + "px";
                this._textContainer.style.verticalAlign = "middle";
                this._textContainer.style.color = "white";
            }
        }
    },
    _updatePath: function() {
        L.Circle.prototype._updatePath.call(this);
        if (!L.Path.CANVAS && this._textLabel) {
            if (L.Path.SVG) {
                this._textLabel.setAttribute("transform", "translate(" + this._point.x + "," + this._point.y + ")");
                this._textLabel.textContent = this._txt;
            }
        }
    },
...
L.Control.ZoomFS = L.Control.Zoom.extend({
    ...
    onAdd: function(map) {
        ...
        this._zoomFSButton = this._createButton('', 'Full Screen', 'leaflet-control-fullscreen', container, this.fullscreen, this);
        this._zoomInButton = this._createButton('+', this.options.zoomInTitle, className + '-in', container, this.zoomInFS, this);
        this._zoomOutButton = this._createButton('-', this.options.zoomOutTitle, className + '-out', container, this.zoomOutFS, this);
        this._updateDisabled();
        map.on('zoomend zoomlevelschange', this._updateDisabled, this);
        return container;
    },
    zoomInFS: function(t) {
        if (typeof this._map.interactionMode != 'undefined' && !this._map.interactionMode) return;
        this._zoomIn(t);
    },
    zoomOutFS: function(t) {
        if (typeof this._map.interactionMode != 'undefined' && !this._map.interactionMode) return;
        this._zoomOut(t);
    },
    setCursor: function(nCursor) {
        SetStyleValue(this._zoomFSButton.style, 'cursor', nCursor);
        SetStyleValue(this._zoomInButton.style, 'cursor', nCursor);
        SetStyleValue(this._zoomOutButton.style, 'cursor', nCursor);
    },
    fullscreen: function() {
        if (typeof this._map.interactionMode != 'undefined' && !this._map.interactionMode) return;
...
L.DomEvent.getMousePosition = function(e, t) {
    var r = document.body,
        i = document.documentElement,
        s = e.pageX ? e.pageX : e.clientX + r.scrollLeft + i.scrollLeft,
        o = e.pageY ? e.pageY : e.clientY + r.scrollTop + i.scrollTop;
    if (typeof window._isFullscreenFix != 'undefined' && window._isFullscreenFix) {
        s -= r.scrollLeft + i.scrollLeft;
        o -= r.scrollTop + i.scrollTop;
    }
    var u = new L.Point(s, o);
    return t ? u._subtract(L.DomUtil.getViewportOffset(t)) : u;
}
_mlCallInitCallback();